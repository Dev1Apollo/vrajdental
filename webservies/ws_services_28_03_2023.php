...
if ($actions == 'adminlogin') {
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        if (validate_password($obj->Password, $good_hash)) {
            ...
            $product = "select productName,lpr,minStockQty,closing from product where closing <= minStockQty and isDelete='0' and istatus='1' ";
            $product = mysqli_query($dbconn, $product);
            if (mysqli_num_rows($product) > 0) {
                while ($rowproduct = mysqli_fetch_assoc($product)) {
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        if (validate_password($obj->Password, $good_hash)) {
//purchaseorder
            $order = "SELECT (select count(*) from purchaseorder where purchaseorder.poStatus = 1 ) as pendingpo , (select count(*) from purchaseorder where purchaseorder.poStatus = 2) as approvedpo,(select count(*) from clinicordermaster where clinicordermaster.orderStatus = 0 ) as PendingOrder,(select count(*) from clinicordermaster where clinicordermaster.orderStatus = 1)as deispatchorder,(select count(*) from product where closing <= minStockQty) as `outofstock`";
