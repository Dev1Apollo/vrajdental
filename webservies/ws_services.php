...
if ($actions == 'adminlogin') {
    ...
    $sql = "select userId,mobile,password,salt,type,contactPerson,deviceToken from user where mobile = '$obj->MobileNo'";
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        if (validate_password($obj->Password, $good_hash)) {
            ...
            $arrayVal = array('productId' => '', 'productName' => '', 'lpr' => '', 'openingStock' => '', 'minStockQty' => '','CurrentStock' => '','venderName' => '');

            $product = "SELECT `productId`,`productName`,`lpr`,`openingStock`,minStockQty,closing as CurrentStock,
                (select vender.venderName from vender inner join purchaseorder on vender.venderId=purchaseorder.venderId 
                inner join purchaseorderdetail on purchaseorderdetail.purchaseOrderId=purchaseorder.purchaseOrderId
                where purchaseorderdetail.productId=product.productId order by purchaseorder.purchaseOrderId desc,purchaseorderdetail.purchaseOrderDetailsId desc limit 1) as venderName
                FROM `product` " . $where . " and isDelete= '0' and istatus= '1' order by productName asc";
            $product = mysqli_query($dbconn, $product);
            if (mysqli_num_rows($product) > 0) {

                if ($row['type'] == "2" || $row['type'] == "3") {
                    $output['ProductList'][] = array('productId' => '0', 'productName' => 'Select Product', 'lpr' => '0', 'openingStock' => '0', 'minStockQty' => '','CurrentStock' => '','venderName' => '');
                }
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        if (validate_password($obj->Password, $good_hash)) {
            ...
            $key = "b34714e10a7fb3840f73415ef1a3975f";
            $mobile = "8401442448";
            $Msg = "Dear Sir, Purchase Order Genrated Successfully . \n\n Please check and approved.";
            $connect->sendWhatsappMessage($mobile,$key,$Msg,'');
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        if (validate_password($obj->Password, $good_hash)) {
            ...
            $product = "SELECT qty,(select product.lpr from product where product.productId = purchaseorderdetail.productId) as lpr,amount,purchaseOrderDetailsId,productId,(select orderdetails.receivedQty from orderdetails where orderdetails.purchaseOrderDetailsId = purchaseorderdetail.purchaseOrderDetailsId) as receivedQty,(select productName from product where product.productId=purchaseorderdetail.productId) as `productName`,DATE_FORMAT(STR_TO_DATE(purchaseorderdetail.strEntryDate, '%d-%m-%Y'),'%d-%m-%Y') as purchaseorderDate FROM `purchaseorderdetail` where  isDelete= '0' and istatus= '1' " . $where . "";
            $product = mysqli_query($dbconn, $product);
            if (mysqli_num_rows($product) > 0) {
                while ($rowproduct = mysqli_fetch_assoc($product)) {
                    $output['PurchaseOrderList'][] = $rowproduct;
